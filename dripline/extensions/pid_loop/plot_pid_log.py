...
for csv_file in csv_files:
    ...
    # Sensor vs Setpoint
    plt.figure()
    plt.plot(times, PV, 'ro', label="PV (sensor)")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{title_str}_sensor.png")
    plt.close()

    # Throttle vs Time
    plt.figure()
    plt.plot(times, throttle, 'go', label="Throttle (actuator)")
    plt.xlabel("Time")
    plt.ylabel("Throttle")
    plt.title(f"{title_str}: Throttle")
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{title_str}_throttle.png")
    plt.close()
