...
class Uint(Data):
    ...
    def __init__(self, value, bits=BYTESIZE):
        ...
        if isinstance(value, int):
            self._from_int(value, bits)
        elif isinstance(value, Data):
            self._from_data(value)
        elif isinstance(value, bytes):
            self._from_bytes(value)
        else:
            raise TypeError(f'invalid type for *value*: {type(value)}')
        
    def _from_int(self, value, bits=BYTESIZE):
        _check_uint(value, bits)
        self._value = value
        self._bits = bits        

    def _from_data(self, value):
        bytes_ = bytes(value)
...
class Float(Data):
    ...
    def _from_float(self, value, bits=4*BYTESIZE):
    ...
    def _from_int(self, value, bits=4*BYTESIZE):
        x = float(value)
...
class Bin(Data):
    ...
    def __init__(self, value, bits=None):
        ...
        if isinstance(value, str):
            self._from_str(value, bits)
        elif isinstance(value, int):
            bits = bits if bits is not None else BYTESIZE
            self._from_int(value, bits)
        elif isinstance(value, Data):
            self._from_data(value)
        elif isinstance(value, bytes):
    ...
    def _from_str(self, value, bits=None):
