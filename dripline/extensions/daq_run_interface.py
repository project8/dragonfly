...
class DAQProvider(core.Service):
    ...
    def __init__(self,
        ...
        if daq_name is None:
            raise core.ThrowReply('DriplineValueError', '<{}> instance <{}> requires a value for "{}" to initialize'.format(self.__class__.__name__, self.name, 'daq_name'))
        else:
            self.daq_name = daq_name
        if run_table_endpoint is None:
            raise core.ThrowReply('DriplineValueError', '<{}> instance <{}> requires a value for "{}" to initialize'.format(self.__class__.__name__, self.name, 'run_table_endpoint'))
        else:
        ...
        if (directory_path is None) and (data_directory_path is None) and (meta_data_directory_path is None):
            raise core.ThrowReply('DriplineValueError', '<{}> instance <{}> requires a value for "{}" to initialize'.format(self.__class__.__name__, self.name, '[meta_[data_]]directory_path'))
    ...
    def run_name(self, value):
        ...
        try:
            result = self.cmd(endpoint=self.run_table_endpoint, specifier='do_insert', keyed_args={'run_name':value})
            ...
            if self._stop_handle is not None:  # end the run
                ...
                self._run_name = None
                self.run_id = None
            raise core.ThrowReply('DriplineValueError', 'failed to insert run_name to the db, obtain run_id, and start_timestamp. run "<{}>" not started\nerror:\n{}'.format(value,str(err)))
    ...
    def end_run(self):
        ...
        if self.run_id is None:
            raise core.ThrowReply('DriplineValueError', "No run to end: run_id is None.")
        self._do_snapshot()
    ...
    def _do_prerun_gets(self):
        ...
        meta_result = self.get(endpoint=self._metadata_state_target, timeout=30)
        self._run_meta.update(meta_result['value_raw'])
        self._run_meta.update({'DAQ_MODE':getattr(self,'acquisition_mode')})
    ...
    def _do_snapshot(self):
        ...
        self.cmd(endpoint=self._snapshot_state_target,
                    specifier='take_snapshot',
                    ordered_args=[self._start_time,time_now,self._metadata_target,filename],
                    timeout=30)
        logger.info('snapshot returned ok')

    def determine_RF_ROI(self):
        raise core.ThrowReply('DriplineMethodNotSupportedError', 'subclass must implement RF ROI determination')
    ...
    def _send_metadata(self, type, data):
        ...
        self.cmd(endpoint=self._metadata_target, specifier='write_json', keyed_args=this_payload)
    ...
    def start_timed_run(self, run_name, run_time):
        ...
        if self._daq_in_safe_mode:
            ...
            raise core.ThrowReply('DriplineDAQNotEnabled', "{} is not enabled: enable it using <dragonfly cmd broadcast.set_condition 0 -b myrna.p8>".format(self.daq_name))

        logger.debug('testing if the DAQ is running')
        result = self.is_running
        if result == True:
            raise core.ThrowReply('DriplineDAQRunning', 'DAQ is already running: aborting run')
    ...
    @property
    def is_running(self):
        raise core.ThrowReply('DriplineMethodNotSupportedError', 'subclass must implement is running method')

    def _do_checks(self):
        raise core.ThrowReply('DriplineMethodNotSupportedError', 'subclass must implement checks methods')

    def _start_data_taking(self,directory,filename):
        raise core.ThrowReply('DriplineMethodNotSupportedError', 'subclass must implement start data taking method')

    def _stop_data_taking(self):
        raise core.ThrowReply('DriplineMethodNotSupportedError', 'subclass must implement stop data taking method')
