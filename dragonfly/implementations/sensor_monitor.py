...
from dripline.core import Gogol, Endpoint, exceptions, constants, fancy_doc
...
class SensorMonitor(Gogol):
    ...
    **NOTE**
    ...
    def __init__(self,
        ...
        kwargs.update({'keys':[key_base+x for x in self.monitors.keys()]})
    ...
    def this_consume(self, message, method):
        ...
        payload.update( { 'timestamp' : message.timestamp } )
        if payload is None:
            logger.warning('Received invalid monitor payload!')
            return
        logger.info('Consuming message from {}'.format(sensor))
        self.process_new_value(self.monitors[sensor], payload)


    def process_new_value(self, monitor, payload):
        ...
        for alarm in monitor['alarms']:
            ...
            if 'max_slope' in alarm.keys():
                x = (datetime.datetime.strptime(payload['timestamp'],constants.TIME_FORMAT)-self.start_time).total_seconds()
    ...
    def configure_monitors(self, sensors):
        for monitor in sensors:
            if 'exchange' not in monitor.keys():
                raise exceptions.DriplineValueError('No exchange provided for {}'.format(monitor))
            ...
            for alarm in monitor['alarms']:
                self.check_monitor_fields(alarm, monitor['exchange'])

        self.monitors = {x.pop('exchange'):x for x in sensors}
        logger.debug('Configured monitors:\n{}'.format(self.monitors))


    def check_monitor_fields(self, alarm, exchange):
        if 'payload_field' not in alarm.keys():
            raise exceptions.DriplineValueError('No payload_field provided for {} : {}'.format(exchange,alarm))
        if 'alarm_message' not in alarm.keys():
            alarm.update({'alarm_message':'{} value raised <{{}}> alarm'.format(exchange)})
        if 'alarm_limit' not in alarm.keys():
            alarm.update({'alarm_limit':False})
        if alarm['alarm_limit']:
            if not isinstance(alarm['alarm_limit'],int):
                raise exceptions.DriplineValueError('Invalid alarm_limit type provided for {} : {}'.format(exchange,alarm))
        ...
        if 'alarm_recurrence' in alarm.keys():
            if not alarm['alarm_recurrence']>0 and isinstance(alarm['alarm_recurrence'],(int,float)):
                raise exceptions.DriplineValueError('Invalid alarm_recurrence provided for {} : {}'.format(exchange,alarm))
            alarm.update({'last_alarm':self.start_time})
        if 'set_condition' in alarm.keys():
            if not isinstance(alarm['set_condition'],int):
                raise exceptions.DriplineValueError('Invalid set_condition type provided for {} : {}'.format(exchange,alarm))
            if alarm['alarm_limit']:
                raise exceptions.DriplineValueError('Invalid alarm_limit setting with set_condition active for {} : {}.'.format(exchange,alarm))
        if 'averaging' not in alarm.keys():
            alarm.update({'averaging':False})
        if alarm['averaging']:
            if not alarm['averaging']>0 and isinstance(alarm['averaging'],int):
                raise exceptions.DriplineValueError('Invalid averaging provided for {} : {}'.format(exchange,alarm))
            alarm.update({'data_record':[]})
        if 'max_value' in alarm.keys():
            self.check_if_float('max_value',alarm,exchange)
        if 'min_value' in alarm.keys():
            self.check_if_float('min_value',alarm,exchange)
        if 'max_slope' in alarm.keys():
            self.check_if_float('max_slope',alarm,exchange)
            if 'slope_points' not in alarm.keys():
                raise exceptions.DriplineValueError('Setting slope_points required with max_slope for {} : {}'.format(exchange,alarm))
            alarm.update({'timeseries':[]})

    def check_if_float(self,key,alarm,exchange):
        if not isinstance(alarm[key],(int,float)):
            try:
                alarm[key] = float(alarm[key])
            except ValueError:
                raise exceptions.DriplineValueError('Invalid {} provided for {} : {}'.format(key,exchange,alarm))
